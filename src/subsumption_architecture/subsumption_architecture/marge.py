...
class PublishFromOSC(Node):
    ...
    def OSC_callback(self,*args):
        ...
        self.osc_big_list.append(osc_list[0])
        self.osc_big_list.append(osc_list[1])
        ...
        '''
        #self.get_logger().info(f'recieve values {time.time()-self.start}')

        if time.time()-self.start >= 0.01:
            pub_list = UInt8MultiArray(data=self.osc_big_list)
            #self.get_logger().info(f'recieve values {time.time()-self.start}')
